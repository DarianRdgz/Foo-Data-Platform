...
const US_STATE_MAP = new Map(US_STATES.map((state) => [state.code, state]));

export function getStateByCode(code: string): UsStateOption | null {
  return US_STATE_MAP.get(code) ?? null;
}

export function isKnownStateCode(code: string): boolean {
  return US_STATE_MAP.has(code);
}